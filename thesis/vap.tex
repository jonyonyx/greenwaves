...
Both master and slave VAP controllers are generated automatically from formal descriptions.
...
These threshold values are also parameters which is fed into the generator routine (see section \ref{dogs} for a description). In the DOGS reference material supplied by TTS vehicle count and occupancy thresholds are identical.
...
When DOGS is active level is adjusted at most by one for each cycle. Thus when the level is high it will take longer to change between levels and the measuring periods are longer, resulting in more accurate measurements.
...
Three intersections in the Glostrup area - Fabriksparken, Gammel Landevej and Kindebjergvej - operate with bus priority on the arterial. 
