\label{evaluation}
All systems in this survey, which use a metaheuristic search, employ a simulator for evaluating solutions in the objective function. Simulations are, in general, widely used to test the final output of some signal optimization procedure in order to compare the new approach to some other, well-known approach eg. TRANSYT. 
...
Simulation is very important in order to establish a search procedure. In table \ref{tab:convergespeed} are the names of three major simulators, which are highly trusted to produce reliable traffic. The Danish Road Directorate prefer VisSim for simulations of infrastructure expansions as well as testing of signal timing plans. In the latter case also TRANSYT is highly regarded and will also perform much faster.
