...
In this section some in-depth discussions are given for three highly adaptive systems, which do not rely on offline optimization. The systems are:

\begin{enumerate}
\item \textbf{RHODES} by Pitu Mirchandani and Larry Head presented in \cite{44}
\item A Phase-by-Phase optimization strategy by Michael Shenoda and Randy Machemehl presented in \cite{1}, in this survey dubbed \textbf{MICMAC}
\item \textbf{DOGS} by danish Technical Traffic Solution (TTS) evaluated in the danish article \cite{dogs}
\end{enumerate}

The first two systems optimize for general traffic networks whereas DOGS (Dynamisk Optimering af Grøntider og Samordning $\sim$ dynamic optimizationg of greens and coordination) optimize for arterials.
